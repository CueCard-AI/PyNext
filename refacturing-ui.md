...
about “designing the app”, they’re thinking about the _shell_.

_Should it have a top nav, or a sidebar?_

_Should the navigation items be on the left, or on the right?_

_Should the page content be in a container, or should it be full-width?_

_Where should the logo go?_

The thing is, an “app” is actually a collection of _features_. Before you’ve
...
_How should this screen look if the user has 2000 contacts?_

_Where should the error message go in this form?_

_How should this calendar look when there are two events scheduled at the
same time?_
...
attachments is going to be _a lot_ more work than you anticipated. There’s no
...
communicate _secure_ and _professional_ , while a trendy new startup might
have a design that feels _fun_ and _playful_.
...
While trying to choose colors using _only_ psychology isn’t super practical — a
lot of it is just about what looks good to you — it can be helpful to think
about when you’re trying to understand _why_ you think a color is the right fit.
...
_Should this text be 12px or 13px?_

_Should this box shadow have a 10% opacity or a 15% opacity?_

_Should this avatar be 24px or 25px tall?_

_Should I use a medium font weight for this button or semibold?_

_Should this headline have a bottom margin of 18px or 20px?_
...
make a decision, _start with a smaller set of options_.
...
picking the initial values _once_ instead of every time you’re designing a new
...
Chances are, two of those options will seem like _obviously_ bad choices. If it’s
...
_Visual hierarchy_ refers to how important the elements in an interface appear
...
While this _does_ reduce the contrast, it often results in text that looks dull,
...
A better approach is to _hand-pick a new color_ , based on the background
color.
...
the element you want to draw attention to, figure out how you can _de-
emphasize_ the elements that are competing with it.
...
easy to fall into the trap of displaying it using a naive _label: value_ format.
...
For example, _janedoe@example.com_ is an email address, _(555) 765- 4321_ is a
phone number and _$19.99_ is a price.

When the format isn’t enough, the context often is. When you see the phrase
_“Customer Support”_ listed below someone’s name in an employee directory,
...
Sometimes you really _do_ need a label; for example when you’re displaying
...
If you’re designing an interface where you know the user will be _looking for_
...
Don’t de-emphasize the data _too_ much in these scenarios; it’s still important
...
Using anh1tag to add a title like _Manage Account_ to a page makes perfect
...
A lot of the time, section titles act more like _labels_ than headings — they are
supportive content, they shouldn’t be stealing all the attention.

Usually the _content_ in that section should be the focus, not the title. That
...
for accessibility reasons but _completely hide_ them visually because the
...
Just like bold text, icons _(especially solid ones)_ are generally pretty “heavy”
...
When designing for the web, white space is almost always _added_ to a design
...
amount of breathing room necessary to not look _actively bad_. To make
something actually look _great_ , you usually need more white space.

Start with too much white space 57


A better approach is to start by giving something _way too much_ space, then
...
_“make sure everything is a multiple of 4px”_ — a naive approach like that
doesn’t make it any easier to choose between 120px and 125px.

For a system to be truly useful, it needs to take into consideration the _relative_
difference between adjacent values.

At the small end of the scale _(like the size of an icon, or the padding inside a
button)_ , a couple of pixels can make a big difference. Jumping from 12px to
16px is an increase of 33%!

But at the large end _(the width of a card, or the vertical spacing in a landing
page hero)_ , a couple of pixels is basically imperceivable. Even increasing the
width of a card from 500px to 520px is only a difference of 4%, which is
_eight times_ less significant than the jump from 12px to 16px.
...
A simple approach is to start with a sensible _base_ value, then build a scale
...
_(sticking to a system is easier when you’re typing in numbers than when
you’re dragging with the mouse.)_
...
designs? These days you’d be hard-pressed to find a _phone_ with a resolution
that low.

So it’s no surprise that when most of us open our design tool of choice on
our high resolution displays, we give ourselves _at least_ 1200-1400px of space
...
need to make _everything_ full-width just because something else (like your
...
But even though grids can be useful, outsourcing _all_ of your layout decisions
...
element’s width is some multiple of 8.33% _(including any gutters)_ , that
element is “on the grid”.

The problem with treating grid systems like a religion is that there are a lot of
situations where it makes much more sense for an element to have a _fixed_
...
the remaining space, using its own _internal_ grid to lay out its children.

Grids are overrated 74


This applies within components, too — don’t use percentages to size
something unless you actually want it to scale.

Grids are overrated 75
...
Grids are overrated 76


The silly thing about this approach is that because column widths are fluid,
there’s a range in screen sizes where the login card is _wider_ on medium
...
relationship by defining your headline size as _2.5em_ ; 2.5 times the current
font size.

There’s nothing inherently wrong with using relative units like _em_ , but don’t
...
_faster_ than elements that are already fairly small — the difference between
...
This works — the buttons _do_ scale up or down and preserve the same
...
Here the large button actually _feels_ like a larger button, and the small
...
make sure there’s more space _around_ the group than there is within it —
...
10px to 24px has been used in the UI _somewhere_.
...
One approach is to calculate your type scale using a _ratio_ , like 4:5 _(a “major
third”)_ , 2:3 _(a “perfect fifth”)_ , or perhaps the “golden ratio”, 1:1.618. This is
often called a “modular scale”.

You start with a sensible base value _(16px is common since it’s the default
font size for most browsers)_ , apply your ratio to get the next value, then
apply your ratio to _that_ value to get the next value, and so on and so forth:
...
2. **You usually need more sizes.**
    ...
    a modular scale are often a bit _too_ limiting.
...
nicely with the spacing and sizing scale recommended in _“Establishing a
spacing and sizing system”_ :
...
When you’re building a type scale, don’t use _em_ units to define your sizes.

Because _em_ units are relative to the current font size, the computed font size
...
If you give an element a font size of 1.25em _(20px by default)_ , inside of that
element 1em is now equal to 20px. That means that if you give one of the
_nested_ elements a font size of .875em, the actual computed font size is
17.5px, not a value from your type scale!

Stick to _px_ or _rem_ units — it’s the only way to guarantee you’re actually
...
This isn’t _always_ true, but as a general rule, typefaces that come in a lot of
different weights tend to be crafted with more care and attention to detail
than typefaces with fewer weights.

Many font directories _(like Google Fonts)_ will let you filter by “number of
...
crank that up to 10+ _(to account for italics)_ :
...
spacing and shorter lowercase letters _(a shorter x-height)_ , while fonts meant
...
There are a lot of great design teams out there full of people with _really_
...
web is using _em_ units, which are relative to the current font size. A width of
...
A better approach is to align mixed font sizes by their _baseline_ , which is the
...
Line-height and font size are _inversely_ proportional — use a taller line-height
...
changing the color _only on hover_.
...
Align with readability in mind 112
...
Align with readability in mind 113
...
Align with readability in mind 114
...
Lowercase letters have a lot of variety visually. Letters like _n_ , _v_ , and _e_ fit
entirely within a typeface’s x-height, other letters like _y_ , _g_ , and _p_ have
_descenders_ that poke out below the baseline, and letters like _b_ , _f_ , and _t_ have
_ascenders_ that extend above.
...
intuitively perceives: _hue_ , _saturation_ , and _lightness_.
...
Don’t confuse HSL for HSB — _lightness_ in HSL is not the same as _brightness_
in HSB.
...
is the same as 100% saturation and _50%_ lightness in HSL.
...
You need more colors than you think 124
...
In practice, you want 8-10 shades to choose from (more on this in “ _Define
your shades up front_ ”). Not so many that you waste time deciding between
...
You need more colors than you think 125


**Primary color(s)**

Most sites need one, _maybe_ two colors that are used for primary actions,
...
Just like with greys, you need a variety _(5-10)_ of lighter and darker shades to
choose from.
...
On top of primary colors, every site needs a few _accent_ colors for
...
You need more colors than you think 126


You might also need colors to emphasize different semantic _states_ , like red
...
You need more colors than you think 127
...
All in, it’s not uncommon to need as many as _ten_ different colors with _5-10
shades each_ for a complex UI.

You need more colors than you think 128
...
“darken” to create shades on the fly. That’s how you end up with 35 _slightly_
...
Start by picking a _base_ color for the scale you want to create — the color in
...
gaps a little more straightforward. Let’s call our darkest shade _900_ , our base
shade _500_ , and our lightest shade _100_.

Start by picking shades _700_ and _300_ , the ones right in the middle of the
gaps. You want these shades to feel like the perfect compromise between
the shades on either side.

This creates four more holes in the scale ( _800_ , _600_ , _400_ , and _200_ ), which
...
Just try to avoid adding _new_ shades too often if you can avoid it. If you’re not
...
But what if your _base_ color is already heavily saturated? How do you increase
...
an inherent _perceived_ brightness due to how the human eye perceives color.
...
_lightness_ component:

While this does work to lighten or darken a color, you often lose some of the
color’s _intensity_ — the color also looks closer to white or to black, not just
lighter or darker.

Since different hues have a different perceived brightness, another way you
can change the brightness of a color is _by rotating its hue_.
...
_color_ in it at all.

But in practice, a lot of the colors that we _think of_ as grey are actually
...
Guidelines (WCAG) recommend that normal text _(under ~18px)_ has a contrast
ratio of at least 4.5:1, and that larger text has acontrast ratioof at least 3:1.

For typical _dark-text-on-a-light-background_ situations, meeting this
...
You can solve this problem by _flipping the contrast_. Instead of using light
...
Even harder than white text on a colored background is _colored_ text on a
...
contrast without getting closer to white is to _rotate the hue_ towards a
...
understand, but be careful not to _rely_ on it, or users with color blindness will
...
In situations like this, try relying on _contrast_ instead of using completely
...
because it’s angled _away_ from the sky, receiving _less_ light.
...
In this case it’s clear that the panels are _inset_ because there’s a shadow at
...
If you want an element to appear raised or inset, first figure out what _profile_
...
Do this by adding a small dark box shadow with a slight vertical offset _(you
only want the shadow to appear below the element)_ :
...
smaller blur radius. It simulates the shadowed area _underneath_ an object
...
created by a lack of ambient light slowly disappears _(go ahead, try it out with
something on your desk)_.
...
It should be quite distinct for your lowest elevation, and almost _(or
completely)_ invisible at your highest elevation.
...
In general _(especially with shades of the same color)_ , lighter objects feel
...
elements to make it feel like a design has multiple _layers_.
...
To solve this problem, you need to _reduce_ the dynamics in the image to
...
lightening or darkening the _whole_ image, not just the problem areas.
...
If you’re designing something that could use some large icons _(like maybe
the “features” section of a landing page)_ , you might instinctively grab your
...
screenshot at a smaller screen size _(like maybe your tablet layout)_ and save a
...
While you’ll always be at your users’ mercy to some extent, there _are_ a few
...
Tables don’t _have_ to work this way, though — if a column doesn’t need to be
...
_“Did the designer do anything here that I never would have thought to do?”_
...
really polished is to recreate that design from scratch, _without peeking at the
developer tools_.
